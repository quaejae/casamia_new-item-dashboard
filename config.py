...
# 전체 요약의 브랜드 배치 순서(여기 없는 신규 브랜드는 뒤에 자동 추가)
...
# 웹 화면 전용 전체요약표 가독성(대시보드 안에서만 적용; PPTX 1슬라이드와 무관)
SUMMARY_WEB_FONT = 13       # px, 기본(헤더·계 등)
SUMMARY_WEB_CELL_FONT = 12  # px, 월셀 프로젝트명/매출
SUMMARY_WEB_PAD = "6px 8px"  # 셀 여백
SUMMARY_WEB_LINE = 1.5      # 줄간격
SUMMARY_WEB_MIN_WIDTH = 1200  # px, 최소 폭(좁으면 가로 스크롤)
